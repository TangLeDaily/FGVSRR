...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_L_OnlypcdThreeLSTM/model_epoch_181_psnr_27.4089.pth", map_location='cpu')
...
def main():
    ...
    # checkpoint = torch.load("checkpoints/over/EDVR_27d17/model_epoch_147_psnr_27.1705.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/DB_EDVR_M/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--DB_EDVR_M--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
