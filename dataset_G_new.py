...
class RandomCrop(object):
    def __init__(self):
        self.scale = 4
        self.output_sizeX = 128
        self.output_sizeY = 128

    def __call__(self, sample):
        lr, hr, ranintX, ranintY = sample['lr'], sample['hr'], sample['ranintX'], sample['ranintY']
        ...
        newlr = lr[:, toplr:toplr + new_h, leftlr: leftlr + new_w]
...
class train_data_set(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'lr': lr, 'hr': hr, 'ranintX': self.randintX[video_ID], 'ranintY': self.randintY[video_ID]}
        lrr, hrr = self.crop(sample)
...
if __name__ == "__main__":
    dataset = train_data_set("datasets/train/", 8)
    training_data_loader = DataLoader(dataset=dataset, batch_size=8, num_workers=0,
                                      drop_last=True)
    for i, batch in enumerate(training_data_loader, 1):
        lr, hr, ID = batch
        save_pic(lr[0, :, :, :], "test/", "lr{}.png".format(i))
        save_pic(hr[0, :, :, :], "test/", "hr{}.png".format(i))
        if i == 900:
            break