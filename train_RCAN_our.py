...
from util import *
from rootmodel.RCAN_SP_1FSR import *
...
def main():
    ...
    checkpoint = torch.load("checkpoints/TEST/RCAN_BIX4.pt", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/RCAN_our/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--RCAN_our--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
