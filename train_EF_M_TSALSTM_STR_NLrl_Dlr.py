...
from rootmodel.EFVSR_M_TSALSTM_STR_NLrl import *
...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_M_TSALSTM_SigToRelu/model_epoch_337_psnr_27.3503.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSALSTM_STR_NLrl_Dlr"
    else:
        str = "ERROR"
    if use_wandb:
    ...
    # checkpoint = torch.load("checkpoints/test/model_epoch_9_psnr_27.2291.pth", map_location='cpu')
    ...
    special_layers_params = list(map(id, model.fusion.parameters()))
    base_params = filter(lambda p: id(p) not in special_layers_params, model.parameters())
    optimizer = optim.Adam([
        {"params": base_params},
        {"params": model.fusion.parameters(), "lr": opt.lr}
    ], lr=opt.lr*0.1)
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSALSTM_STR_NLrl_Dlr/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSALSTM_STR_NLrl_Dlr--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
