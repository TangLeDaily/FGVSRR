...
from rootmodel.EFVSR_M_TSALSTM_DSA_NoAdd_CC import *
...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_M_TSALSTM_DSAdd_CellConv/model_epoch_471_psnr_27.3687.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSALSTM_DSA_NoAdd_CC"
    else:
        str = "ERROR"
    if use_wandb:
    ...
    checkpoint = torch.load("checkpoints/EFVSR_M_TSALSTM_DSA_NoAdd_CC/model_epoch_89_psnr_27.3575.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSALSTM_DSA_NoAdd_CC/"
    else:
...
def test_train_set(this_model, test_loader, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSALSTM_DSA_NoAdd_CC--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
