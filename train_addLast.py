...
from rootmodel.SPVSR_addLast import *
...
def get_yu(model):
    ...
    torch.save(kk.state_dict(), "checkpoints/state/New_1.pth")

    pretrained_dict = torch.load("checkpoints/state/New_1.pth")
...
def main():
    ...
    checkpoint = torch.load("checkpoints/SPVSR_GNew/model_epoch_125_psnr_26.1414.pth")
...
def train(optimizer, model, criterion, epoch, train_dataloader):
    ...
    for iteration, batch in enumerate(train_dataloader):
        ...
        feat_ID = iteration // opt.frame + 300
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/SPVSR_AddLast/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--SPVSR_addLast--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
