...
from dataset_E_deblur import *
from util import *
from model import *
from srresnet import *
from rootmodel.edvr_deblur import *
...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_M_TSALSTM_DotSkipAttn/model_epoch_296_psnr_27.3560.pth", map_location='cpu')
...
def bufen(model):
    ...
    model_dict = model.state_dict()
    checkpoint = torch.load("checkpoints/EFVSR_M_TSALSTM_DotSkipAttn/model_epoch_296_psnr_27.3560.pth", map_location='cpu')
...
def main():
    ...
    if opt.model_mark == 0:
        ...
        str = "ERROR"
    if use_wandb:
        wandb.init(project="DeBlurring", name=str, entity="karledom")
    print(opt)
    ...
    checkpoint = torch.load("checkpoints/test/model_epoch_1_psnr_30.6434.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/edvr_deblur/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--edvr_deblur--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
