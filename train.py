...
def main():
    ...
    training_data_loader = DataLoader(dataset=train_set, batch_size=opt.batchSize, num_workers=opt.threads, drop_last=True)
    ...
    for iteration, batch in enumerate(train_dataloader):
        ...
        if iteration % opt.frame == 0:
            for j in range(opt.batchSize):
                print("del:{}".format(str(j + iteration//opt.frame*opt.batchSize)))
                del_train_feat(str(j + iteration//opt.frame))
        input, target, feat_ID, picname = batch
        print("feat_ID")
        print(feat_ID)
        print("picname")
        print(picname)
        ...
        # if iteration % 200 == 0:
        #     if use_wandb:
        #         wandb.log({'epoch': epoch, 'iter_loss': avr_loss.avg})
        #     print('epoch_iter_loss is {:.10f}'.format(avr_loss.avg))
...
def save_checkpoint(model, epoch):
    ...
    torch.save(model.state_dict(), "checkpoints/SPVSR/state/model_epoch_{}_state.pth".format(epoch))
...
def test_total_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--SPVSR_G--epoch:{}--TotalAvg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num, psnr.avg))
