...
from rootmodel.SPVSR_attention_Sav import *
...
def get_yu(model):
    kk = torch.load("checkpoints/SPVSR_GNew/model_epoch_125_psnr_26.1414.pth")
...
def train(optimizer, model, criterion, epoch, train_dataloader):
    ...
    for iteration, batch in enumerate(train_dataloader):
        ...
        feat_ID = iteration // opt.frame + 500
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/SPVSR_attn_sav/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--SPVSR_attn_sav--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
