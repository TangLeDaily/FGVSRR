...
from rootmodel.EFVSR_M_OnlypcdThreeLSTM_adRef import *
...
parser.add_argument("--batchSize", type=int, default=32, help="training batch size")  # default 16
...
parser.add_argument("--lr", type=float, default=0.00008, help="Learning Rate. Default=1e-4")
...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_L_OnlypcdThreeLSTM/model_epoch_170_psnr_27.3950.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_OnlypcdThreeRFLSTM_V1"
    else:
        str = "ERROR"
    if use_wandb:
    ...
    checkpoint = torch.load("checkpoints/EFVSR_M_OnlypcdThreeRFLSTM_V1/model_epoch_324_psnr_27.3018.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_OnlypcdThreeLSTM_adRef/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_OnlypcdThreeLSTM_adRef--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
