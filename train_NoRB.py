...
from rootmodel.SPVSR_NoRB import *
...
def get_yu(model):
    kk = torch.load("checkpoints/SPVSR_G/model_videoID_TOTA_epoch_12.pth")
    torch.save(kk.state_dict(), "checkpoints/state/R_12.pth")

    pretrained_dict = torch.load("checkpoints/state/R_12.pth")
...
def main():
    ...
    if opt.model_mark == 0:
        model = SPVSR_G(in_c=3, out_c=3, mid_c=64, kernel_size=3, pre_RN=5)
    # else:
    #     model = NLVR()
    # 加载模型
