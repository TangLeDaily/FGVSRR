...
from rootmodel.EFVSR_M_TSA3LSTM_CellNLAdd import *
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSA3LSTM_CellNLAdd"
    else:
        str = "ERROR"
    if use_wandb:
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSA3LSTM_CellNLAdd/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSA3LSTM_CellNLAdd--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
