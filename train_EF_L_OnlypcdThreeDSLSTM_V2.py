...
from rootmodel.EFVSR_L_OnlypcdThreeDSLSTM_V2 import *
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_L_OnlypcdThreeDSLSTM_V2"
    else:
        str = "ERROR"
    if use_wandb:
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_L_OnlypcdThreeDSLSTM_V2/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_L_OnlypcdThreeDSLSTM_V2--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
