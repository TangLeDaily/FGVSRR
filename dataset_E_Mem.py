...
class train_data_set(data.Dataset):
    def __init__(self, rootpath, batchsize): #datasets/train/
        ...
        self.picLr = []
        self.picHr = []
        for i in range(len(self.input_path_ls)):
            miniLr = []
            miniHr = []
            for j in range(len(self.pic_name_lis)):
                miniLr.append(load_img(self.input_path_ls[i] + self.pic_name_lis[j]))
                miniLr.append(load_img(self.target_path_ls[i] + self.pic_name_lis[j]))
            self.picLr.append(miniLr)
            self.picHr.append(miniHr)
            print("视频：{} 已经加载完毕".format(i))
    ...
    def __getitem__(self, idx):
        ...
        # print(len(self.input_path_ls))，，，，，，，，，
        # print("len:{}--idx:{}--videoID:{}--frame:{}".format(len(self.input_path_ls) * len(self.pic_name_lis), idx, video_real_ID, frame))
        lrC = self.transLR(self.picLr[video_real_ID][frame])
        if frame == 0:
            lrN = self.transLR(self.picLr[video_real_ID][frame+1])
        else:
            lrN = self.transLR(self.picLr[video_real_ID][frame-1])
        hr = self.transHR(self.picHr[video_real_ID][frame])
...
class test_data_set(data.Dataset):
    def __init__(self, rootpath, path): #datasets/test/  and 000/
        ...
        self.picLr = []
        self.picHr = []
        for i in range(len(self.pic_name_lis)):
            self.picLr.append(load_img(self.input_path + self.pic_name_lis[i]))
            self.picHr.append(load_img(self.target_path + self.pic_name_lis[i]))
        self.transLR = transforms.Compose([
            transforms.ToTensor()])
        self.transHR = transforms.Compose([
            transforms.ToTensor()])
    ...
    def __getitem__(self, idx):
        lrc = self.transLR(self.picLr[idx])
        if idx == 0:
            lrn = self.transLR(self.picLr[idx+1])
        else:
            lrn = self.transLR(self.picLr[idx-1])
        hr = self.transHR(self.picHr[idx])
