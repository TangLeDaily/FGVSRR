...
parser.add_argument('--input_path', type=str,
                    default='datasets/enhance/test/input/BasketballPass_50/',
                    help='input path to use')
parser.add_argument('--output_path', default='datasets/enhance/test/input/BasketballPass_50/',
                    type=str,
                    help='where to save the output image')
parser.add_argument('--gt_path', type=str,
                    default='datasets/enhance/test/target/BasketballPass_50/',
                    help='gt path to use')
parser.add_argument('--model', type=str, default='checkpoints/dct/best_model.pth',  #  original/epoch_1290_min_batch_loss_0.01916392535592119
                    help='model file to use')
parser.add_argument('--calc_on_y', default=True, action='store_true', help='calc on y channel')
...
for i in listdir(input_path):
    if is_image_file(i):
        with torch.no_grad():
            ...
            cv.imwrite('{}input/{}.png'.format(opt.output_path, "%03d" % index), input_center_img)  # input_center_img is real

            out_img = output_tensor_full.mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).type(
                torch.uint8).numpy()

            if opt.calc_on_y is True:
                input_center_img = cv.cvtColor(input_center_img, cv.COLOR_BGR2YUV)  # input_center_img is real
