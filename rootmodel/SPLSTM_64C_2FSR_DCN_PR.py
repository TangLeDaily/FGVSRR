...
class feLSTM(nn.Module):
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.add_h_gate = nn.Conv2d(input_size + hidden_size, hidden_size, 3, 1, 1)
        self.add_h_dcn = DCNv2Pack(
            input_size,
            input_size,
            3,
            stride=1,
            padding=1,
            deformable_groups=8)
        self.add_i_gate = nn.Conv2d(input_size + hidden_size, hidden_size, 3, 1, 1)
        self.add_i_dcn = DCNv2Pack(
            input_size,
            input_size,
            3,
            stride=1,
        ...
        self.out_dcn = DCNv2Pack(
            ...
            padding=1,
            deformable_groups=8)
        # self.last = nn.Conv2d(hidden_size, hidden_size, 3, 1, 1)
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        forget_offset = self.forget_gate(stacked_inputs)
        cell_add = torch.sigmoid(self.forget_dcn(prev_cell, forget_offset)) * prev_cell

        add_h_offset = self.add_h_gate(stacked_inputs)
        add_h = torch.sigmoid(self.add_h_dcn(prev_hidden, add_h_offset))
        add_i_offset = self.add_i_gate(stacked_inputs)
        add_i = self.lrelu(self.add_i_dcn(input_, add_i_offset))

        cell = cell_add + add_h * add_i

        o_offset = self.out_gate(stacked_inputs)
        hidden = torch.tanh(cell) * self.lrelu(self.out_dcn(input_, o_offset))
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            feat, next_h, next_c = self.LSTM(feat, None, None)
        else:
            feat, next_h, next_c = self.LSTM(feat, last_h, last_c)
