...
class DSLSTM(nn.Module):
    ...
    def __init__(self, input_size = 64, hidden_size = 64, crop_num = 4):
        ...
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.Gates = nn.Conv2d(input_size + hidden_size * 4, 5 * hidden_size, 3, padding=3 // 2)
        self.attn = SELayer(channel = 5 * hidden_size)
    ...
    def forward(self, input_, np_feat, h_suv, np_feat_c, c_suv):
        ...
        stacked_inputs = torch.cat((input_, prev_hidden[0], prev_hidden[1], prev_hidden[2], prev_hidden[3]), 1)
        gates = self.Gates(stacked_inputs)
        ...
        # chunk across channel dimension
        start, in_gate, remember_gate, out_gate, cell_gate = gates.chunk(5, 1)
        ...
        # compute current cell and hidden state
        cell = (remember_gate * prev_cell) + (in_gate * cell_gate)
        hidden = out_gate * f.tanh(cell) + start
