...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        if pre_h is None:
            ...
            outNfeat, next_hn, next_cn = self.LSTMn(aligned_feat[:, 1, :, :, :].clone(), None, None)
        else:
            outRfeat, next_hr, next_cr = self.LSTM(aligned_feat[:, 0, :, :, :].clone(), pre_h[0], pre_c[0])
            outNfeat, next_hn, next_cn = self.LSTMn(aligned_feat[:, 0, :, :, :].clone(), pre_h[1], pre_c[1])
        next_h.append(next_hr)
        next_h.append(next_hn)
        next_c.append(next_cr)
        next_c.append(next_cn)
