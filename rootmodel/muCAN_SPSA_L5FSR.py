...
class Attention(nn.Module):
    ...
    def forward(self, x, last_h, last_c, SP=False):
        ...
        if SP:
          x, next_h, next_c = self.SPM(x, last_h, last_c)
        else:
          next_h = x
          next_c = x
...
class CNCAM(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        # out_ref, next_h, next_c = self.SPM(down_fea, last_h, last_c)
        # down_fea = self.fusion(torch.cat((down_fea, out_ref), dim=1))

        # print("down_fea: {}".format(down_fea.size()))
        ...
        att_fea,_,_ = self.spa_l[0](down_fea,None,None,SP=False)
        all_f = [att_fea]
        next_h = []
        next_c = []
        for j in range(self.nl):
            ...
            if last_h is None:
                att_sim_f, next_h_m, next_c_m = self.spa_l[j + 1](sim_f, None, None, SP=True)
                next_h.append(next_h_m)
                next_c.append(next_c_m)
            else:
                att_sim_f, next_h_m, next_c_m = self.spa_l[j + 1](sim_f, last_h[j], last_c[j])
                next_h.append(next_h_m)
                next_c.append(next_c_m)
...
class TemporalFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        emb_ref = self.ref_conv(x[:, 0, :, :, :].clone())
...
class Network(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            fea, next_h, next_c = self.cncam(fea, None, None)
        else:
            fea, next_h, next_c = self.cncam(fea, last_h, last_c)
        ...
        bic = F.interpolate(x_center, scale_factor=4, mode='bicubic', align_corners=False)
        out = bic + residual
