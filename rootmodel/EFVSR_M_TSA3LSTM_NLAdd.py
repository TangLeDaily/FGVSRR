...
class feLSTM(nn.Module):
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        for i in range(self.lstm_num):
            if prev_hidden is None:
                hidden, cell = self.LSTM[str(i)](input_, None, None)
            else:
                hidden, cell = self.LSTM[str(i)](input_, prev_hidden[i], prev_cell[i])
            h_feat.append(hidden)
            if i == 0:
                next_h.append(hidden)
                next_c.append(cell)
        for j in range(self.lstm_num-1):
            if prev_hidden is not None:
                next_h.append(prev_hidden[j])
                next_c.append(prev_cell[j])
            else:
                next_h.append(next_h[0])
                next_c.append(next_c[0])
        out = input_
        for k in range(self.lstm_num):
            attn_feat, _ = self.attn(input_, h_feat[k])
            out = out + 0.5 * attn_feat
