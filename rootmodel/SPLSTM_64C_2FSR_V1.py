...
class feLSTM(nn.Module):
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.c_offset_conv1 = nn.Conv2d(input_size + hidden_size, hidden_size, 3,  1, 1)
        self.c_offset_conv2 = nn.Conv2d(hidden_size, hidden_size, 3, 1, 1)
        self.c_dcn = DCNv2Pack(
            hidden_size,
            hidden_size,
            3,
            stride=1,
        ...
        self.h_offset_conv1 = nn.Conv2d(input_size + hidden_size, hidden_size, 3, 1, 1)
        self.h_offset_conv2 = nn.Conv2d(hidden_size, hidden_size, 3, 1, 1)
        self.h_dcn = DCNv2Pack(
            hidden_size,
            hidden_size,
            3,
            stride=1,
        ...
        self.c_x_conv = nn.Conv2d(input_size, hidden_size, 3, 1, 1)
        self.h_x_conv = nn.Conv2d(input_size, hidden_size, 3, 1, 1)

        self.last = nn.Conv2d(hidden_size*2, input_size, 3, 1, 1)
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        offset_c = self.lrelu(self.c_offset_conv2(self.lrelu(self.c_offset_conv1(torch.cat((prev_cell, input_), dim=1)))))
        cell_1 = self.lrelu(self.c_dcn(prev_cell, offset_c))

        offset_h = self.lrelu(self.h_offset_conv2(self.lrelu(self.h_offset_conv1(torch.cat((prev_cell, input_), dim=1)))))
        hidden_1 = torch.sigmoid(self.c_dcn(prev_hidden, offset_h))

        cell_2 = cell_1 * self.lrelu(self.c_x_conv(input_))
        hidden_2 = hidden_1 * self.lrelu(self.h_x_conv(input_))

        out = self.lrelu(self.last(torch.cat((cell_2, hidden_2), dim=1)))

        return out, hidden_2, cell_2
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        aligned_feat[:, 0, :, :, :], next_h, next_c = self.LSTM(aligned_feat[:, 0, :, :, :].clone(), pre_h, pre_c)
        # temporal attention
