...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            bf_feat, next_h, next_c = self.LSTM(feat_l1, None, None)
        else:
            bf_feat, next_h, next_c = self.LSTM(feat_l1, last_h, last_c)
        ...
        bf_feat = bf_feat.view(b, t, -1, h, w)
        ...
        for i in range(t):
            ...
            if i == 0:
                aligned_feat.append(feat + bf_feat[:, 0, :, :, :])
            else:
