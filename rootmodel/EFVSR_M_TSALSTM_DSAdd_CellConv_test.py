...
class ConvGRUCell(nn.Module):
    ...
    def __init__(self, input_size=64, hidden_size=64, kernel_size=3):
        super().__init__()
...
class feLSTM(nn.Module):
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.Other_Gates = nn.Conv2d(input_size + hidden_size, 3 * hidden_size, 3, padding=3 // 2)
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        # data size is [batch, channel, height, width]
        stacked_inputs = torch.cat((input_, prev_hidden), 1)
        gates = self.Other_Gates(stacked_inputs)
        out = torch.sigmoid(self.Out_Gates(stacked_inputs))

        # chunk across channel dimension
        in_gate, remember_gate, cell_gate = gates.chunk(3, 1)
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        embedding_ref = embedding_ref # self.fusion_LS(torch.cat((embedding_ref, outLfeat), dim=1))
