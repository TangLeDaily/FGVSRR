...
class DeformableConvLSTMCell(tf.keras.layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        if self.first is True:
            ...
            self.wconvInput = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                          name="wconvInput");

            kernel_shape = [inputs.shape[1], inputs.shape[2], self._depth]
            kernel_shape = tf.TensorShape(kernel_shape);
            self.gateBias = tf.Variable(lambda: self.gateBiasInitializer(shape=kernel_shape, dtype=tf.float32),
                                        dtype=dtype, name="inputBias");

            kernel_shape = self._kernel + [hidden_prev.shape[-1], self._depth]
            kernel_shape = tf.TensorShape(kernel_shape);
            self.wconvHidden = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                           name="wconvHidden");

            kernel_shape = [self._depth]
            kernel_shape = tf.TensorShape(kernel_shape);
            self.wxi = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="wxi");
            self.whi = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="whi");
            self.inputBias = tf.Variable(lambda: self.inputBiasInitializer(shape=kernel_shape, dtype=tf.float32),
                                         dtype=dtype, name="inputBias");

            self.wxf = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="wxf");
            self.whf = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="whf");
            self.forgetBias = tf.Variable(lambda: self.forgetBiasInitializer(shape=kernel_shape, dtype=tf.float32),
                                          dtype=dtype, name="forgetBias");

            self.wxo = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="wxo");
            self.who = tf.Variable(lambda: self._initializer(shape=kernel_shape, dtype=tf.float32), dtype=dtype,
                                   name="who");
            self.outputBias = tf.Variable(lambda: self.outputBiasInitializer(shape=kernel_shape, dtype=tf.float32),
                                          dtype=dtype, name="outputBias");
        ...
        if self.counter % 16 in self.frames:
            ...
            outInputs = DeformableConvLayer2D(filters=self._depth, kernel_size=(self._kernel[0], self._kernel[1]),
                                              kernel_initializer=self._initializer)(inputs);
        else:
