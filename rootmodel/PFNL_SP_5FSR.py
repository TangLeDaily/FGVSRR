...
class NonLocalBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, sub_sample=1, nltype=0):
        ...
        self.SPM = SPM(in_channels, in_channels)
    ...
    def forward(self, input_x, last_h, last_c):
        ...
        # b,63, h*w//64, h*w
        SPM_in = theta_x.permute(0, 2, 1).view(f_b, f_c, height, width)
        SPM_out, next_h, next_c = self.SPM(SPM_in, last_h, last_c)
        SPM_in = self.fusion_LS(torch.cat((SPM_in, SPM_out), dim=1))
        theta_x = SPM_in.view(f_b, f_c, f_s).permute(0, 2, 1)
