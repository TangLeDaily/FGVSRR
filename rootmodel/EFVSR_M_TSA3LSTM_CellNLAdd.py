...
class NLAttentionBlock(nn.Module):
    def __init__(self, scale=1, useUpDown = True):
        super(NLAttentionBlock, self).__init__()
        self.useUpDown = useUpDown
        if useUpDown:
    ...
    def forward(self, ref, neb):
        ...
        if self.useUpDown:
            ...
            align_feat = torch.stack((ref, neb), dim=1)
            align_feat = self.attn(align_feat)
            out_ref = align_feat[:, 0, :, :, :].clone()
            out_neb = align_feat[:, 1, :, :, :].clone()
...
class feLSTM(nn.Module):
    ...
    Generate a convolutional LSTM cell
    """
    def __init__(self, input_size=64, cell_num=3,  hidden_size=64):
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        for i in range(self.cell_num):
            if prev_cell is not None:
                cell, _ = self.attn(prev_cell[0], prev_cell[i])
                if i == 0:
                    center_cell = cell
                else:
                    center_cell = center_cell + cell
        out, next_h, next_cc = self.LSTM(input_, prev_hidden, center_cell)
        next_c = []
        next_c.append(next_cc)
        for j in range(self.cell_num-1):
            if prev_cell is None:
                next_c.append(next_cc)
            else:
                next_c.append(prev_cell[j])

        return out + input_, next_h, next_c
...
class feeLSTM(nn.Module):
    ...
    def __init__(self, input_size=64, lstm_num=3, hidden_size=64):
        super(feeLSTM, self).__init__()
        self.lstm_num = lstm_num
        self.LSTM = LSTM()
        self.conv = nn.Conv2d(input_size * lstm_num, input_size, 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, input_, prev_hidden, prev_cell):  # c[0] 当前， c[1]前一个， c[2]前二个
        if prev_hidden is None:
            next_h, next_c_1 = self.LSTM(input_, None, None)
        else:
            for i in range(len(prev_cell)):
                if i == 0:
                    c = prev_cell[i]
                else:
                    c = c + prev_cell[i]
            next_h, next_c_1 = self.LSTM(input_, prev_hidden, c)
        next_c = []
        next_c.append(next_c_1)
        if prev_hidden is not None:
            for j in range(len(prev_cell)):
                if j < 2:
                    next_c.append(prev_cell[j])
                else:
                    break
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        outLfeat, next_h, next_c = self.LSTM(aligned_feat[:, 0, :, :, :].clone(), pre_h, pre_c)
        aligned_feat[:, 0, :, :, :] = outLfeat.clone()
        # temporal attention
