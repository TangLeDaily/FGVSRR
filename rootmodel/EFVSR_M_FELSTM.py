...
class feLSTM(nn.Module):
    ...
    def __init__(self, input_size=64, hidden_size=64, cell_num = 4):
        super(feLSTM, self).__init__()
        self.cell_num = cell_num
        self.LSTM_first = LSTM()
        self.LSTM_after = nn.ModuleDict()
        for i in range(cell_num):
            self.LSTM_after[str(i)] = LSTM()
        # self.conv = nn.Conv2d(input_size*2, input_size, 3, 3//2, 1)
        # self.RB = ResidualBlockNoBN()
        # self.relu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, input_, prev_hidden, prev_cell):
        hidden, cell = self.LSTM_first(input_, prev_hidden, prev_cell)
        after_h = hidden
        after_c = cell
        for i in range(self.cell_num):
            after_h, after_c = self.LSTM_after[str(i)](input_, after_h, after_c)

        return after_h, hidden, cell
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            feat_l1, next_h, next_c = self.FELSTM(feat_l1, None, None)
        else:
            feat_l1, next_h, next_c = self.FELSTM(feat_l1, last_h, last_c)
