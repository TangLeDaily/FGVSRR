...
class SPM_Attention(nn.Module):
    def __init__(self, nf=64):
        super(SPM_Attention, self).__init__()
        self.sAtt_1 = nn.Conv2d(nf, nf, 1, 1, bias=True)
        self.max_pool = nn.MaxPool2d(3, stride=2, padding=1)
        self.avg_pool = nn.AvgPool2d(3, stride=2, padding=1)
        self.sAtt_2 = nn.Conv2d(nf * 2, nf, 1, 1, bias=True)
...
class CNCAM(nn.Module):
    def __init__(self, nf=64, n_level=3):
        ...
        self.spa_l.append(SPM_Attention(nf))
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            att_fea, next_h, next_c = self.spa_l[0](down_fea, last_h, last_c, SP=True)
        else:
            att_fea, next_h, next_c = self.spa_l[0](down_fea, last_h, last_c, SP=True)
        all_f = [att_fea]
        for j in range(self.nl):
