...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.InConv = nn.Conv2d(input_size, input_size, 3, 1, 1)
        self.OutConv = nn.Conv2d(input_size, input_size, 3, 1, 1)
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        input_ = torch.tanh(self.InConv(input_))
        ...
        out = self.lrelu(self.OutConv(hidden))
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, center_feat):
        b, t, c, h, w = aligned_feat.size()
        # temporal attention
        embedding_ref = self.temporal_attn1(center_feat.clone())
        embedding = self.temporal_attn2(aligned_feat.view(-1, c, h, w))
        embedding = embedding.view(b, t, -1, h, w)  # (b, t, c, h, w)

        corr_l = []  # correlation list
        for i in range(t):
            emb_neighbor = embedding[:, 1, :, :, :]
...
class EFVSR(nn.Module):
    def __init__(self,
        ...
        self.feLSTM = nn.ModuleDict()
        for i in range(3):
            self.feLSTM[str(i)] = LSTM()
    ...
    def forward(self, x, last_h, last_c):
        ...
        for i in range(3):
            if last_h is None:
                out_feat, next_hh, next_cc = self.feLSTM[str(i)](ref_feat_l[i], None, None)
            else:
                out_feat, next_hh, next_cc = self.feLSTM[str(i)](ref_feat_l[i], last_h[i], last_c[i])
            next_h.append(next_hh)
            next_c.append(next_cc)
            out_feat_l.append(out_feat)

        aligned_feat = []
        LSfeat = self.pcd_align(out_feat_l, ref_feat_l)
        aligned_feat.append(LSfeat)
        ...
        ref_feat_nbr = [  # neighboring feature list
            feat_l1[:, 0, :, :, :].clone(),
            feat_l2[:, 0, :, :, :].clone(),
            feat_l3[:, 0, :, :, :].clone()
        ]
        center_feat_l = self.pcd_align(ref_feat_nbr, ref_feat_l)
        feat = self.fusion(aligned_feat, center_feat_l)
