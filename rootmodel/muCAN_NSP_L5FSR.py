...
class TemporalFusion(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        emb_ref = self.ref_conv(x[:, self.n_frame-1, :, :, :].clone())
        emb_ref, next_h, next_c = self.SPM(emb_ref, last_h, last_c)
        # emb_ref = self.fusion(torch.cat((emb_ref, out_ref), dim=1))
        ...
        for i in range(N):
            ...
            cor = torch.sum(emb_neighbor * emb_ref, dim=1, keepdim=True)
            cor_l.append(cor)
...
class TMCAM(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        out, next_h, next_c = self.tf(aggr_fea, last_h, last_c)
...
class Network(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        x_center = x[:, self.n_frame-1, :, :, :].contiguous()
        ...
        if last_h is None:
            fea, next_h, next_c = self.tmcam([L1_fea, L2_fea, L3_fea], None, None)
        else:
            fea, next_h, next_c = self.tmcam([L1_fea, L2_fea, L3_fea], last_h, last_c)
        fea = self.cncam(fea)
