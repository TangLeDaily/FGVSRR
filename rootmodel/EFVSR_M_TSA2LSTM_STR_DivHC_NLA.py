...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        next_c = []
        if pre_h is None:
            outRfeat, next_hr, next_cr = self.LSTM(aligned_feat[:, 0, :, :, :].clone(), None, None)
            outNfeat, _, _ = self.LSTMn(aligned_feat[:, 1, :, :, :].clone(), next_hr, next_cr)
        else:
            outRfeat, next_hr, next_cr = self.LSTM(aligned_feat[:, 0, :, :, :].clone(), pre_h[0], pre_c[0])
            outNfeat, _, _ = self.LSTMn(aligned_feat[:, 1, :, :, :].clone(), pre_h[1], pre_c[1])
        next_h.append(next_hr)
        next_c.append(next_cr)
        if pre_h is None:
            next_h.append(next_hr)
            next_c.append(next_cr)
        else:
            next_h.append(pre_h[0])
            next_c.append(pre_c[0])
        aligned_feat[:, 1, :, :, :] = aligned_feat[:, 1, :, :, :].clone() + outNfeat
        print("outRfeatin -- size:{}".format(outRfeat.size()))
        outRfeat, outNfeat = self.attn(outRfeat, outNfeat)
        print("outRfeat -- size:{}".format(outRfeat.size()))
        aligned_feat[:, 1, :, :, :] = aligned_feat[:, 1, :, :, :].clone() + outNfeat
        embedding_ref = self.temporal_attn1(
            aligned_feat[:, 0, :, :, :].clone() + outRfeat)
