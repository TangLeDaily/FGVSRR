    ...
    def forward(self, x, last_h, last_c):
        ...
        for i in range(t):
            ...
            if i == 0:
                ...
                for j in range(3):
                    if last_h is None:
                        lstm_f, next_hh, next_cc = self.feLSTM[str(j)](nbr_feat_l[j], None, None)
                    else:
                        lstm_f, next_hh, next_cc = self.feLSTM[str(j)](nbr_feat_l[j], last_h[j], last_c[j])
                    next_h.append(next_hh)
                    next_c.append(next_cc)
                    lstm_feat.append(lstm_f)
                feat = self.pcd_align(nbr_feat_l, ref_feat_l)
            aligned_feat.append(feat)
        lsfeat = self.pcd_align(lstm_feat, ref_feat_l)
