...
class LSTMAlign(nn.Module):
    def __init__(self, num_feat=64, deformable_groups=8):
        ...
        self.TSA_1 = TSAFusion()
        self.TSA_2 = TSAFusion()
        self.TSA_3 = TSAFusion()

        self.fusion_conv = nn.Conv2d(num_feat*3, num_feat, 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
    ...
    def forward(self, x, last_h, last_c):
        ...
        x_center_3 = h3
        for i in range(4):
            neb_feat_3.append(self.DCN_align_3(neb_feat_2[i], x_center_2))

        fusion_1 = []
        fusion_2 = []
        fusion_3 = []
        fusion_1.append(x_center_1+x_center)
        fusion_2.append(x_center_2+x_center)
        fusion_3.append(x_center_3+x_center)
        for i in range(4):
            fusion_1.append(neb_feat_1[i])
            fusion_2.append(neb_feat_2[i])
            fusion_3.append(neb_feat_3[i])
        out1 = self.TSA_1(torch.stack(fusion_1, dim=1))
        out2 = self.TSA_2(torch.stack(fusion_2, dim=1))
        out3 = self.TSA_3(torch.stack(fusion_3, dim=1))

        # out.size(): (b, c, h, w)
        out = self.lrelu(self.fusion_conv(torch.cat((out1, out2, out3), dim=1)))
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        out, new_h, new_c = self.LSTMAlign(feat, last_h=last_h, last_c=last_c)
