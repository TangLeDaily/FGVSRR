...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        return feat, start_embedding, next_h, next_c
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c):
        ...
        if last_h is None:
            feat, out_2, next_h, next_c = self.fusion(aligned_feat, None, None)
        else:
            feat, out_2, next_h, next_c = self.fusion(aligned_feat, last_h, last_c)
        ...
        # out_h = self.reconstruction(next_c)
        out_h = self.lrelu(self.pixel_shuffle(self.upconv1(feat)))
        out_h = self.lrelu(self.pixel_shuffle(self.upconv2(out_h)))
        out_h = self.lrelu(self.conv_hr(out_h))
        out_h = self.conv_last(out_h)

        return out, out_h, next_h, next_c
