...
class feLSTM(nn.Module):
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        if Not_Attn:
            ...
            cell, _ = self.attn((remember_gate * prev_cell) + (in_gate * cell_gate), prev_cell)
        # compute current cell and hidden state
        hidden = out_gate * torch.tanh(cell)
        return hidden, hidden, cell

class FeatCollect(nn.Module):
    def __init__(self, weight=0.8):
        super(FeatCollect, self).__init__()
        self.attn = NLAttentionBlock()
        self.weight = weight
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
    def forward(self, x, pre):
        if pre is None:
            ...
            pre = pre.detach()
            attn_pre, _ = self.attn(pre, x)
            attn_pre = torch.tanh(attn_pre)
            add_sum = self.weight * pre + x
            out_attn = attn_pre * add_sum
            out_attn = self.lrelu(out_attn)
            next = add_sum + out_attn
            out = out_attn + x
            return out, next
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c, pre_fc):
        ...
        aligned_feat, next_fc = self.collect(aligned_feat.view(-1, c, h, w), pre_fc)
        embedding = self.temporal_attn2(aligned_feat)
        ...
        return feat, next_h, next_c, next_fc
...
class EFVSR(nn.Module):
    ...
    def forward(self, x, last_h, last_c, last_fc):
        b, t, c, h, w = x.size()
        ...
        if last_h is None:
            feat, next_h, next_c, next_fc = self.fusion(aligned_feat, None, None, None)
        else:
            feat, next_h, next_c, next_fc = self.fusion(aligned_feat, last_h, last_c, last_fc)
        ...
        out += base
        return out, next_h, next_c, next_fc
