    ...
    def forward(self, x, last_h, last_c):
        ...
        for i in range(t):
            ...
            if i == 0:
                ...
                next_c = []
                for j in range(3):
                    print("j:{}--{}".format(j, nbr_feat_l[j].size()))
                    if last_h is None:
                        nbr_feat_l[j], next_hh, next_cc = self.feLSTM[str(j)](nbr_feat_l[j], last_h=None, last_c=None)
                    else:
                        nbr_feat_l[j], next_hh, next_cc = self.feLSTM[str(j)](nbr_feat_l[j], last_h, last_c)
