...
class DSLSTM(nn.Module):
    ...
    def forward(self, input_, np_feat, h_suv, np_feat_c, c_suv):
        ...
        prev_cell = prev_cell.cuda()
        out, prev_cell = self.LSTM["0"](input_, prev_hidden[0], prev_cell)
        out, prev_cell = self.LSTM["1"](out, prev_hidden[1], prev_cell)
        out, prev_cell = self.LSTM["2"](out, prev_hidden[2], prev_cell)
        out, prev_cell = self.LSTM["3"](out, prev_hidden[3], prev_cell)
        out += input_
        return out, prev_cell
