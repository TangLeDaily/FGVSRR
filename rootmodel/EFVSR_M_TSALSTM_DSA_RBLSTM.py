...
class LSTMResidualBlockNoBN(nn.Module):
    def __init__(self, num_feat=64, res_scale=1, pytorch_init=False):
        super(LSTMResidualBlockNoBN, self).__init__()
        self.res_scale = res_scale
        self.conv1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
        self.LSTM = LSTM()
    ...
    def forward(self, x, h, c):
        identity = x
        out1, n_h, n_c = self.LSTM(x, h, c)
        out2 = self.conv2(self.relu(self.conv1(x)) + out1)
        out = identity + out2 * self.res_scale
        return out, n_h, n_c
...
class EFVSR(nn.Module):
    def __init__(self,
        ...
        self.num_RB = num_reconstruct_block
        ...
        # reconstruction2
        self.LSTM_reconstruction = nn.ModuleDict()
        for i in range(num_reconstruct_block):
            self.LSTM_reconstruction[str(i)] = LSTMResidualBlockNoBN()
        # upsample
    ...
    def forward(self, x, last_h, last_c, RB_h, RB_c):
        b, t, c, h, w = x.size()
        ...
        for k in range(self.num_RB):
            if RB_h is None:
                feat, next_RB_mh, next_RB_mc = self.LSTM_reconstruction[str(k)](feat, None, None)
            else:
                feat, next_RB_mh, next_RB_mc = self.LSTM_reconstruction[str(k)](feat, RB_h[k], RB_c[k])
            next_RB_h.append(next_RB_mh)
            next_RB_c.append(next_RB_mc)
        ...
        out += base
        return out, next_h, next_c, next_RB_h, next_RB_c
