...
class DSLSTM(nn.Module):
    ...
    def __init__(self, input_size=64, hidden_size=64):
        ...
        self.h1_conv = nn.Conv2d(hidden_size, hidden_size // 4, 3, padding=3 // 2)
        self.h2_conv = nn.Conv2d(hidden_size, hidden_size // 4, 3, padding=3 // 2)
        self.h3_conv = nn.Conv2d(hidden_size, hidden_size // 4, 3, padding=3 // 2)
        self.h4_conv = nn.Conv2d(hidden_size, hidden_size // 4, 3, padding=3 // 2)
    ...
    def forward(self, input_, np_feat, h_suv, np_feat_c, c_suv):
        ...
        for i in range(4):
            if h_suv[i]:
                prev_hidden.append(np_feat[i].cuda())
            else:
                prev_hidden.append(Variable(torch.zeros(state_size)).cuda())

        prev_hidden = prev_hidden
        prev_cell = prev_cell.cuda()
        h1 = f.relu(self.h1_conv(prev_hidden[0]))
        h2 = f.relu(self.h2_conv(prev_hidden[1]))
        h3 = f.relu(self.h3_conv(prev_hidden[2]))
        h4 = f.relu(self.h4_conv(prev_hidden[3]))
        # data size is [batch, channel, height, width]
        stacked_inputs = torch.cat((input_, h1, h2, h3, h4), 1)
...
class PCDAlignment(nn.Module):
    ...
    def __init__(self, num_feat=64, deformable_groups=8):
        ...
        for i in range(3, 0, -1):
            ...
            self.DSLSTM[level] = DSLSTM(num_feat, num_feat)
    ...
    def get_feature_PCD(self, feat_ID, i):
        ...
        h_suv = []
        np_feat = []
        if os.path.exists("data/feature/" + feat_ID + "_{}_pcd_h1.npy".format(i)):
            h_suv.append(True)
            np_feat_h1 = numpy.load("data/feature/" + feat_ID + "_{}_pcd_h1.npy".format(i))
            np_feat.append(torch.tensor(np_feat_h1))
        else:
            np_feat.append(123)
            h_suv.append(False)
        if os.path.exists("data/feature/" + feat_ID + "_{}_pcd_h2.npy".format(i)):
            h_suv.append(True)
            np_feat_h2 = numpy.load("data/feature/" + feat_ID + "_{}_pcd_h2.npy".format(i))
            np_feat.append(torch.tensor(np_feat_h2))
        else:
            np_feat.append(123)
            h_suv.append(False)
        if os.path.exists("data/feature/" + feat_ID + "_{}_pcd_h3.npy".format(i)):
            h_suv.append(True)
            np_feat_h3 = numpy.load("data/feature/" + feat_ID + "_{}_pcd_h3.npy".format(i))
            np_feat.append(torch.tensor(np_feat_h3))
        else:
            np_feat.append(123)
            h_suv.append(False)
        if os.path.exists("data/feature/" + feat_ID + "_{}_pcd_h4.npy".format(i)):
            h_suv.append(True)
            np_feat_h4 = numpy.load("data/feature/" + feat_ID + "_{}_pcd_h4.npy".format(i))
            np_feat.append(torch.tensor(np_feat_h4))
        else:
            np_feat.append(123)
            h_suv.append(False)
        if os.path.exists("data/feature/" + feat_ID + "_{}_pcd_c.npy".format(i)):
            np_feat_c = torch.tensor(numpy.load("data/feature/" + feat_ID + "_{}_pcd_c.npy".format(i)))
            c_suv = True
        else:
            np_feat_c = numpy.zeros(5)
            c_suv = False
        return np_feat, h_suv, np_feat_c, c_suv

    def save_feature_PCD(self, feat_ID, j, np_feat, h_suv, feat_h, feat_c):
        if not os.path.exists("../data/feature/"):
            os.makedirs("../data/feature/")
        sav_feat = []
        for i in range(3):
            if h_suv[i + 1]:
                numpy.save("data/feature/" + feat_ID + "_{}_pcd_h{}.npy".format(j, i + 1),
                           np_feat[i + 1].cpu().detach().numpy())
        sav_feat.append(feat_h.cpu().detach().numpy())
        numpy.save("data/feature/" + feat_ID + "_{}_pcd_h{}.npy".format(j, 4), feat_h.cpu().detach().numpy())

        np_feat_c = feat_c.cpu().detach().numpy()
        numpy.save("data/feature/" + feat_ID + "_{}_pcd_c.npy".format(j), np_feat_c)
    ...
    def forward(self, feat_ID, nbr_feat_l, ref_feat_l, sav):
        ...
        for i in range(3, 0, -1):
            ...
            if sav:
                np_feat, h_suv, np_feat_c, c_suv = self.get_feature_PCD(feat_ID, i - 1)
                feat, next_c = self.DSLSTM[level](feat, np_feat, h_suv, np_feat_c, c_suv)
                self.save_feature_PCD(feat_ID, i - 1, np_feat, h_suv, feat, next_c)
            if i < 3:
