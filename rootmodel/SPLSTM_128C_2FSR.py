...
class feLSTM(nn.Module):
    def __init__(self, input_size=64, hidden_size=128):
        ...
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.Add_Gates = nn.Conv2d(input_size + hidden_size, hidden_size + hidden_size, 3, padding=3 // 2)
        ...
        if self.input_size != self.hidden_size:
            self.Last_Gates = nn.Conv2d(hidden_size, input_size, 3, 1, 1)
    ...
    def forward(self, input_, prev_hidden, prev_cell):
        ...
        if self.input_size != self.hidden_size:
            hidden_out = self.lrelu(self.Last_Gates(hidden))
        else:
            hidden_out = hidden
        # cell = cell + self.lrelu(self.enhence(prev_cell))
        return hidden_out, hidden, cell
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat, pre_h, pre_c):
        ...
        outLfeat, next_h, next_c = self.LSTM_128C(embedding_ref, pre_h, pre_c)
