...
class NLLSTM(nn.Module):
    def __init__(self):
        super(NLLSTM, self).__init__()
        self.LSTM = LSTM()
        self.attn = NLAttentionBlock()

    def forward(self, x, pre_h, pre_c):
        out, next_h, next_c = self.LSTM(x, pre_h, pre_c)
        out, _ = self.attn(x, out)
...
class RBLSTM(nn.Module):
    def __init__(self, num_feat=64, res_scale=1, pytorch_init=False):
        super(RBLSTM, self).__init__()
        self.res_scale = res_scale
        self.NLLSTM = NLLSTM()
        self.conv1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, pre_h, pre_c):
        ...
        out, next_h, next_c = self.NLLSTM(out, pre_h, pre_c)

        return identity + out * self.res_scale, next_h, next_c
...
class EFVSR(nn.Module):
    def __init__(self,
        ...
        self.num_lstm_blocks = num_lstm_block
        ...
        # reconstruction2
        self.LSTMblocks = nn.ModuleDict()
        for num in range(num_lstm_block):
            self.LSTMblocks[str(num)] = RBLSTM()
    ...
    def forward(self, x, last_h, last_c):
        ...
        next_h = []
        next_c = []
        for num in range(self.num_lstm_blocks):
            if last_h is None:
                feat, n_h, n_c = self.LSTMblocks[str(num)](feat, None, None)
            else:
                feat, n_h, n_c = self.LSTMblocks[str(num)](feat, last_h[num], last_c[num])
            next_h.append(n_h)
            next_c.append(n_c)
