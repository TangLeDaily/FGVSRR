...
class EFModel(nn.Module):
    def __init__(self,
        ...
        self.preb = make_layer(
            ResidualBlockNoBN, preblock, num_feat=in_c)
        self.outb = make_layer(
            ResidualBlockNoBN, outblock, num_feat=in_c)

    def get_feature(self, feat_ID):
        if not os.path.exists("data/feature/"):
            os.makedirs("data/feature/")
        if os.path.exists("data/feature/" + feat_ID + "_result.npy"):
            np_feat = numpy.load("data/feature/" + feat_ID + "_result.npy")
            return True, np_feat
        else:
            return False, None
    def forward(self, x, feat_ID):
        start = x
        out = self.preb(x)
        Nofirst, last = self.get_feature(feat_ID)
        if Nofirst:
            out += last
        out = self.outb(out)
        out += start
...
class PCDAlignment(nn.Module):
    ...
    def forward(self, feat_ID, nbr_feat_l, ref_feat_l, sav):
        # Pyramids
        if sav:
            last_feat = self.get_feature(feat_ID) #0 ， 1 ， 2 （大， 中， 小）
            for i in range(3, 0, -1):
                nbr_feat_l[i-1] += last_feat[i-1]
...
class EFVSR(nn.Module):
    ...
    def get_feature(self, feat_ID):
        ...
        if os.path.exists("data/feature/" + feat_ID + ".npy"):
            np_feat_h = numpy.load("data/feature/" + feat_ID + "_h.npy")
            np_feat_c = numpy.load("data/feature/" + feat_ID + "_c.npy")
    ...
    def save_feature(self, feat_ID, feat):
        if not os.path.exists("data/feature/"):
            os.makedirs("data/feature/")
        np_feat = feat.cpu().detach().numpy()
        numpy.save("data/feature/" + feat_ID + "_result.npy", np_feat)
    def forward(self, x, feat_ID):
        feat_ID = str(feat_ID)
        b, t, c, h, w = x.size()
        ...
        next_h = self.enhence(first[:, 1, :, :, :], feat_ID)
