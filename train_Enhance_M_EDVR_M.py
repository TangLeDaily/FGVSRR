...
def main():
    ...
    checkpoint = torch.load("checkpoints/edvr_enhance/model_epoch_32_psnr_26.3080.pth", map_location='cpu')
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(test_loader, 1):
            ...
            if opt.cuda:
                ...
                target = target.cuda()
            out = model(input)
            # base_psnr.update(calc_psnr(input[:,0,:,:,:], target), len(out))
            psnr.update(calc_psnr(out, target), len(out))
        if use_wandb:
            wandb.log({'epoch': epoch_num, 'psnr': psnr.avg})
        print("--->This--edvr_enhance--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 27.9984 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
