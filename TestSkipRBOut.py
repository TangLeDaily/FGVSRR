...
from rootmodel.EFVSR_M_TSALSTM_DSAdd_SkipRB import *
...
def main():
    ...
    checkpoint = torch.load("checkpoints/over/TSALSTM_DSAdd/model_epoch_123_psnr_27.3620.pth", map_location='cpu')
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(test_loader, 1):
            ...
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            out, out_h, last_h, last_c = model(input, last_h=last_h, last_c=last_c)
            save_pic(out[0,:,:,:], "data/out/C/out/", "{}_out.png".format(iteration))
            save_pic(out_h[0,:,:,:], "data/out/C/out_h/", "{}_out_h.png".format(iteration))
