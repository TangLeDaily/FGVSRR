...
from dataset_E_denoise import *
from util import *
from rootmodel.edvr_denoise import *

parser = argparse.ArgumentParser(description="PyTorch Data_Pre")
parser.add_argument("--train_root_path", default='datasets/denoise/train/', type=str, help="train root path")
parser.add_argument("--test_root_path", default='datasets/denoise/test/', type=str, help="test root path")
...
def get_yu(model):
    kk = torch.load("checkpoints/edvr_denoise/model_epoch_12_psnr_33.4322.pth", map_location='cpu')
...
def main():
    ...
    if opt.model_mark == 0:
        ...
        str = "ERROR"
    if use_wandb:
        wandb.init(project="Denoise", name=str, entity="karledom")
    print(opt)
    ...
    checkpoint = torch.load("checkpoints/edvr_denoise/model_epoch_8_psnr_33.2438.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/edvr_denoise/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--edvr_denoise--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
