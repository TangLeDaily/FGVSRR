...
parser.add_argument("--lr", type=float, default=2.0971520000000012e-05, help="Learning Rate. Default=1e-4")
...
def main():
    ...
    # checkpoint = torch.load("/home/tangle/code/FGVSR/checkpoints/edvr_5/model_epoch_160_psnr_27.5339.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EDVR_N5FSR/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EDVR_N5FSR--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
