...
from rootmodel.EFVSR_M_OnlypcdThreeLSTM_pre import *
...
def get_yu(model):
    kk = torch.load("checkpoints/DB_EDVR_M/model_epoch_124_psnr_27.3071.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_OnlypcdThreeLSTM_pre"
    else:
        str = "ERROR"
    if use_wandb:
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_OnlypcdThreeLSTM_pre/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_OnlypcdThreeLSTM_pre--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
