...
from dataset_New_N5 import *
from util import *
from rootmodel.EDVR_SP_N5_Pre import *

use_wandb = True
Project_name = "SPVSR_plus"
This_name = "EDVR_SP_pre_N5FSR"

parser = argparse.ArgumentParser(description="PyTorch Data_Pre")
parser.add_argument("--train_root_path", default='datasets/train_plus/', type=str, help="train root path")
...
parser.add_argument("--lr", type=float, default=4e-5, help="Learning Rate. Default=1e-4")
...
def get_yu3(model):
    kk = torch.load("checkpoints/state/EDVR_M_x4_SR_REDS_official-32075921.pth")
    kk = kk['params']
    model_dict = model.state_dict()
    kk = {k: v for k, v in kk.items() if k in model_dict}
    model_dict.update(kk)  # 利用预训练模型的参数，更新模型
...
def main():
    ...
    state_dict = torch.load('checkpoints/New_Over/PreTrain/EDVR_N5_SP/epoch_218_psnr_30.5394_lr_4.611686018427394e-06.pth', map_location='cpu')
