...
from util import *
from rootmodel.edvr_deblur_our import *
...
def get_yu(model):
    kk = torch.load("checkpoints/edvr_M_322/model_epoch_38_psnr_27.2841.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/edvr_deblur_our/"
    else:
...
def test_train_set(this_model, test_loader, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--edvr_deblur_our--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
