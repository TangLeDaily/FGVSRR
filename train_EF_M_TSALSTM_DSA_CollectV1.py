...
from rootmodel.EFVSR_M_TSALSTM_DSA_CollectV1 import *
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSALSTM_DSA_CollectV1"
    else:
        str = "ERROR"
    if use_wandb:
...
def train(optimizer, model, criterion, epoch, train_dataloader):
    ...
    for iteration, batch in enumerate(train_dataloader):
        ...
        if opt.cuda:
            input = input.cuda()
            target = target.cuda()
        out, last_h, last_c, last_fc = model(input, last_h=last_h, last_c=last_c, last_fc=last_fc)
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSALSTM_DSA_CollectV1/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSALSTM_DSA_CollectV1--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
