...
from util import *
from rootmodel.EFVSR_M_TSALSTM_DSA_AddThenDot import *
...
def get_yu(model):
    kk = torch.load("checkpoints/EFVSR_M_TSALSTM_DSA_NoAdd_CC/model_epoch_44_psnr_27.3676.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSALSTM_DSA_AddThenDot"
    else:
        str = "ERROR"
    if use_wandb:
    ...
    # checkpoint = torch.load("checkpoints/EFVSR_M_TSALSTM_DSA_NoAdd_CC/model_epoch_1_psnr_27.3653.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSALSTM_DSA_AddThenDot/"
    else:
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(test_loader, 1):
            ...
            if opt.cuda:
                ...
                target = target.cuda()
            out, last_h, last_c = model(input, last_h=last_h, last_c=last_c)
            save_pic(out[0, :, :, :], "data/out/000/", "{}_out.png".format(iteration))
            psnr.update(calc_psnr(out, target), len(out))
        if use_wandb:
            wandb.log({'epoch': epoch_num, 'psnr': psnr.avg})
        print("--->This--EFVSR_M_TSALSTM_DSA_AddThenDot--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
