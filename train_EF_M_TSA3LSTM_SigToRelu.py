...
from rootmodel.EFVSR_M_TSA3LSTM_SigToRelu import *
...
def get_yu(model):
    kk = torch.load("checkpoints/over/EDVR_M_27d32/model_epoch_325_psnr_27.3234.pth", map_location='cpu')
...
def main():
    ...
    str = "None"
    if opt.model_mark == 0:
        str = "EFVSR_M_TSA3LSTM_SigToRelu"
    else:
        str = "ERROR"
    if use_wandb:
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSA3LSTM_SigToRelu/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSA3LSTM_SigToRelu--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
