...
from rootmodel.EFVSR_M_TSA3LSTM_NLAdd import *
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_TSA3LSTM_NLAdd/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_TSA3LSTM_NLAdd--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
