...
from rootmodel.EFVSR_M_withLSTM_replace import *
...
def get_yu(model):
    kk = torch.load("checkpoints/best/model_epoch_34_psnr_26.7068.pth", map_location='cpu')
...
def main():
    ...
    # checkpoint = torch.load("checkpoints/EFVSR_M_withLSTM_add/model_epoch_1_psnr_27.2025.pth", map_location='cpu')
...
def save_checkpoint(model, psnr, epoch):
    ...
    if opt.model_mark == 0:
        model_folder = "checkpoints/EFVSR_M_withLSTM_replace/"
    else:
...
def test_train_set(this_model, epoch_num):
    ...
    with torch.no_grad():
        ...
        print("--->This--EFVSR_M_withLSTM_replace--epoch:{}--Avg--PSNR: {:.4f} dB--Root--PSNR: 24.11 dB".format(epoch_num,
                                                                                               psnr.avg))
    return psnr.avg
