...
def add_noise(img):
    ...
    kkk = skimage.util.random_noise(img, mode="gaussian", var=0.001, mean=0)
    kkk = kkk*255
    img = Image.fromarray(kkk.astype('uint8'))#.convert('RGB')
    return img
...
class RandomCrop(object):
    def __init__(self):
        # self.scale = 4
        self.output_sizeX = 256
        self.output_sizeY = 256
    def __call__(self, sample):
        ...
        new_hh, new_ww = self.output_sizeX, self.output_sizeY
        tophr = toplr
        lefthr = leftlr
...
class train_data_set(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if video_real_ID > 239:
            video_real_ID = video_real_ID-240
